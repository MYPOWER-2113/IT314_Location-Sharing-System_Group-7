...
import '../Signin_Signup/utils.dart';
...
class _SOS extends State<SOS> {
  ...
  void _sendSOS(List<String> recepitent) async {
    ...
    //     Uri.parse('${Constants.uri}/getSOSnumber'),
    //     // body: jsonEncode({'email': user.email}),
    //     headers: <String, String>{
    //       'Content-Type': 'application/json; charset=UTF-8',
    //     },);
    //
    //   final List<String> numbersList = List<String>.from(jsonDecode(res.body));
    //
    //   if(numbersList.isEmpty)
    //   {
    //     Navigator.push(
    //         context, new MaterialPageRoute(builder: (context) => sosContact()));
    //   }
    //   else
    //   {
    //     recepients = numbersList;
    //     await sendSMS(message:"Hi there this is message from my app. My location is https://www.google.com/maps/@$lat,$long,20z",recipients:numbersList);
    //   }
    //
    // } catch (e) {
    //   showSnackBar(context, /*e.toString()*/ 'Hello');
    // }
    ...
    await sendSMS(
        message:
            "Hi there this is message from my app. My location is https://www.google.com/maps/@$lat,$long,20z",
        recipients: recepitent);
  }
  ...
  Widget build(BuildContext context) {
    if (isSwitched) if (recepients.isEmpty) /*return sosContact();*/
    fetchMobileNumbers();
...
void fetchMobileNumbers() async {
  ...
  try {
    ...
    print(res.body);
    print(email);
    List<String> numbersList = List<String>.from(jsonDecode(res.body));
